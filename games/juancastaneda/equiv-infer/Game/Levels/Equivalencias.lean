import Game.Levels.Equivalencias.L10
...
Para ejecutar algún teorema sobre una ocurrencia distinta a la primera se escribiría '{teorema} pos {numero_de_ocurrencia}'. Por ejemplo, para ejecutar la definición de la implicación en la segunda ocurrencia, se escribiría 'definicion_implicacion pos 2'. Si tienes una expresión como '(p → q) → (p → r)' y le quisieras aplicar la definición de la implicación al '(p → r)', escribirías 'definicion_implicacion pos 3', pues el primer '→' es el de la expresión general que conecta '(p → q)' con '(p → r)', el segundo sería el de '(p → q)' y finalmente el tercero sería el de '(p → r)'.
